<table class="table table-light mt-5 table-hover table-responsive-sm table-borderless radius-10">
    <thead>
    ...
    <tbody>
        ...
        <tr class="text-center">
            ...
            <td class="bg-dark text-light border border-light">{{$sold->piece}}</td>
            @if(Request::segment(1) != 'sellers')
            <td class="bg-danger text-white border border-light" onclick="undo(`{{$sold->id}}`)"><i
                    class="ion-arrow-return-left"></i></td>
            @endif
        </tr>
        @endforeach
    </tbody>
